...
metrics = ["SDR", "PEMOQ", "PEAQ", "time"];
ylabels = ["SDR (dB)", "ODG", "ODG", "time (s)"];
% ftitles = ["peak SDR", "peak ODG by PEMO-Q", "peak ODG by PEAQ"];
ftitles = ["SDR", "ODG by PEMO-Q", "ODG by PEAQ", "time"];
...
for i = 1:length(metrics)
    ...
    title(tls, {ftitles(i), "lighter shade, solid: arburg", "darker shade, dashed: lpc"})
    ...
    for m = 1:length(methods)
        ...
        for s = 1:length(signals)
            for g = 1:length(gaps)
                for p = 1:length(ps)
                    ...
                    % find maximum
                    if iscell(tables.(methods(m)).(metrics(i)))
                        ...
                        data(s, g, p, 1) = tables.(methods(m)).(metrics(i))(burgrow);
                        if lpcrow
                            data(s, g, p, 2) = tables.(methods(m)).(metrics(i))(lpcrow);
                        end
                    end
                end
            end
        ...
        for p = 1:length(ps)
            if intervals
                ...
                h(p) = plot(gaps, means(:, p, 1), "Color", 0.6 + 0.4*colors(p, :));
                plot(gaps, means(:, p, 2), "Color", colors(p, :), "LineStyle", "--")
            end
