...
% candidates to plot
orders = [2048, 2048, 1024, 512, 2048, 1024, NaN, NaN];
algos = ["arburg", "arburg", "arburg", "arburg", "arburg", "arburg", "", ""];
...
methodnames = [...
    ...
    sprintf("Etter, p = %d, %s", orders(5), algos(5)), ...
    sprintf("Etter PLC, p = %d, %s", orders(6), algos(6)), ...
    "A-SPAIN", ...
    "A-SPAIN-MOD" ...
    ];
...
orders = orders(selection);
algos = algos(selection);
methodnames = methodnames(selection);
...
%% add SPAIN
S = load(fold + "/results_spain.mat", "tables");
    ...
    for m = 1:length(methods)
        for s = 1:length(signals)
            for g = 1:length(gaps)
                ...
                if contains(methods(m), ["janssen", "etter", "extrapolation"]) % only AR methods
